...
class Deck:
	...
	def _deal(self, num):
		...
		cards = self.cards[-actual:] #Removing the cards
		self.cards = self.cards[:-actual] #Updating the deck of cards
		return cards #returns the list of cards

	def deal_card(self):
		return self._deal(1)[0] #instead of returning a list of card, it returns a card
